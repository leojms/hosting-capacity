...
def define_3ph_pvsystem(dss, bus, kv, kva, pmpp):
    ...
    dss.text(
        "New XYCurve.MyEff npts=4  xarray=[.1  .2  .4  1.0]  yarray=[1 1 1 1]")
...
def add_bus_marker(dss, bus, color, size_marker=8, code=15):
    dss.text("AddBusMarker bus={} color={} size={} code={}".format(
        bus, color, size_marker, code))
...
def increment_pv_size(dss, p_step, kva_to_kw, pf, i):
    ...
    for _ in range(dss.pvsystems_count()):
        dss.text(
            f"edit pvsystem.{dss.pvsystems_read_name()} pmpp={p_step * i} kva={kva_to_kw * p_step * i} pf={pf} pfpriority=yes")
        dss.pvsystems_next()
    dss.solution_solve()
